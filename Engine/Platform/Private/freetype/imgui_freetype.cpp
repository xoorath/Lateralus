...
// Original code by @vuhdo (Aleksei Skriabin). Improvements by @mikesart. Maintained since 2019 by
// @ocornut.

// CHANGELOG
// (minor and older changes stripped away, please see git history for details)
//  2023/08/01: added support for SVG fonts, enable by using '#define IMGUI_ENABLE_FREETYPE_LUNASVG'
//  (#6591) 2023/01/04: fixed a packing issue which in some occurrences would prevent large amount
//  of glyphs from being packed correctly. 2021/08/23: fixed crash when FT_Render_Glyph() fails to
//  render a glyph and returns NULL. 2021/03/05: added ImGuiFreeTypeBuilderFlags_Bitmap to load
//  bitmap glyphs. 2021/03/02: set 'atlas->TexPixelsUseColors = true' to help some backends with
//  deciding of a prefered texture format. 2021/01/28: added support for color-layered glyphs via
...
// FIXME: cfg.OversampleH, OversampleV are not supported (but perhaps not so necessary with this
// rasterizer).

// Modifications by xoorath:
// - Body of this file is conditional on ENABLE_IMGUI
// - Replacing IM_ASSERT with LAT_ASSERT
#if ENABLE_IMGUI
...
struct GlyphInfo
{
    ...
    FT_Int OffsetY; // The distance from the origin to the top of the glyph. This is usually a value
                    // < 0.
    float AdvanceX; // The distance from the origin to the origin of the next glyph. This is usually
                    // a value > 0.
...
struct FreeTypeFont
{
    bool InitFont(FT_Library ft_library, const ImFontConfig &cfg,
                  unsigned int extra_user_flags); // Initialize from an external data buffer.
                                                  // Doesn't copy data, and you must ensure it stays
                                                  // valid up to this object lifetime.
    void CloseFont();
                            ...
                            unsigned int extra_font_builder_flags)
{
    FT_Error error = FT_New_Memory_Face(ft_library, (uint8_t *)cfg.FontData,
                                        (uint32_t)cfg.FontDataSize, (uint32_t)cfg.FontNo, &Face);
    if (error != 0)
        return false;
...
void FreeTypeFont::SetPixelHeight(int pixel_height)
{
    ...
    // ImGui assumes that the 'pixel_height' is a maximum height of an any given glyph, i.e. it's
...
const FT_Glyph_Metrics *FreeTypeFont::LoadGlyph(uint32_t codepoint)
{
    ...
    // You can use FreeType 2.10, or the patched version of 2.11.0 in VcPkg, or probably any
    // upcoming FreeType version.
...
#ifdef IMGUI_ENABLE_FREETYPE_LUNASVG
    LAT_ASSERT(slot->format == FT_GLYPH_FORMAT_OUTLINE || slot->format == FT_GLYPH_FORMAT_BITMAP ||
                   slot->format == FT_GLYPH_FORMAT_SVG);
#else
#if ((FREETYPE_MAJOR >= 2) && (FREETYPE_MINOR >= 12))
    LAT_ASSERT_MSG(slot->format != FT_GLYPH_FORMAT_SVG,
                             ...
                             unsigned char *multiply_table)
{
    ...
    case FT_PIXEL_MODE_MONO: // Monochrome image, 1 bit per pixel. The bits in each byte are ordered
                             // from MSB to LSB.
    {
...
#undef DE_MULTIPLY
    ...
    default: LAT_ASSERT_MSG(0, "FreeTypeFont::BlitGlyph(): Unknown bitmap pixel mode!");
    }
}
} // namespace
...
{
    LAT_ASSERT(atlas->ConfigData.Size > 0);
    ...
    for (int src_i = 0; src_i < atlas->ConfigData.Size; src_i++)
    {
        ...
        LAT_ASSERT(src_tmp.DstIndex != -1); // cfg.DstFont not pointing within atlas->Fonts[] array?
             ...
             src_range += 2)
        {
            ...
            // isn't persistent.
            LAT_ASSERT(src_range[0] <= src_range[1]);
        ...
        for (const ImWchar *src_range = src_tmp.SrcRanges; src_range[0] && src_range[1];
            ...
            for (int codepoint = src_range[0]; codepoint <= (int)src_range[1]; codepoint++)
            {
                ...
                uint32_t glyph_index = FT_Get_Char_Index(
                    src_tmp.Font.Face, codepoint); // It is actually in the font? (FIXME-OPT: We are
                                                   // not storing the glyph_index..)
                if (glyph_index == 0)
                    continue;
    ...
    for (int src_i = 0; src_i < src_tmp_array.Size; src_i++)
    {
        ...
        LAT_ASSERT(sizeof(src_tmp.GlyphsSet.Storage.Data[0]) == sizeof(ImU32));
            ...
            if (ImU32 entries_32 = *it)
                for (ImU32 bit_n = 0; bit_n < 32; bit_n++)
                    if (entries_32 & ((ImU32)1 << bit_n))
                    {
                        ...
                        // src_glyph.GlyphIndex = 0; // FIXME-OPT: We had this info in the previous
                        // step and lost it..
                        src_tmp.GlyphsList.push_back(src_glyph);
                    }
        src_tmp.GlyphsSet.Clear();
        LAT_ASSERT(src_tmp.GlyphsList.Size == src_tmp.GlyphsCount);
    }
    ...
    // Hopefully users of FreeType won't mind the temporary allocations.
    ...
    for (int src_i = 0; src_i < src_tmp_array.Size; src_i++)
    {
        ...
        for (int glyph_i = 0; glyph_i < src_tmp.GlyphsList.Size; glyph_i++)
        {
            ...
            LAT_ASSERT(buf_bitmap_current_used_bytes + bitmap_size_in_bytes <=
                      BITMAP_BUFFERS_CHUNK_SIZE); // We could probably allocate custom-sized buffer
                                                  // instead.
    ...
    stbrp_init_target(&pack_context, atlas->TexWidth - atlas->TexGlyphPadding,
                      TEX_HEIGHT_MAX - atlas->TexGlyphPadding, pack_nodes.Data, pack_nodes.Size);
    ...
    for (int src_i = 0; src_i < src_tmp_array.Size; src_i++)
    {
        ...
        for (int glyph_i = 0; glyph_i < src_tmp.GlyphsCount; glyph_i++)
        {
            ...
            LAT_ASSERT(pack_rect.was_packed);
            if (pack_rect.w == 0 && pack_rect.h == 0)
                continue;

            GlyphInfo &info = src_glyph.Info;
            LAT_ASSERT(info.Width + padding <= pack_rect.w);
            LAT_ASSERT(info.Height + padding <= pack_rect.h);
            ...
            dst_font->AddGlyph(&cfg, (ImWchar)src_glyph.Codepoint, x0, y0, x1, y1, u0, v0, u1, v1,
                               info.AdvanceX);

            ImFontGlyph *dst_glyph = &dst_font->Glyphs.back();
            LAT_ASSERT(dst_glyph->Codepoint == src_glyph.Codepoint);
...
static void *FreeType_Realloc(FT_Memory /*memory*/, long cur_size, long new_size, void *block)
{
    ...
    {
        void *new_block =
            GImGuiFreeTypeAllocFunc((size_t)new_size, GImGuiFreeTypeAllocatorUserData);
...
static FT_Error ImGuiLunasvgPortRender(FT_GlyphSlot slot, FT_Pointer *_state)
{
    ...
    // If there was an error while loading the svg in ImGuiLunasvgPortPresetSlot(), the renderer
    // hook still get called, so just returns the error.
...
static FT_Error ImGuiLunasvgPortPresetSlot(FT_GlyphSlot slot, FT_Bool cache, FT_Pointer *_state)
{
    ...
    // ImGuiLunasvgPortRender(). If it's the latter, don't do anything because it's // already done
    // in the former.
    if (cache)
        return state->err;
    ...
    // Compute all the bearings and set them correctly. The outline is scaled already, we just need
    ...
    double vertBearingX =
        slot->metrics.horiBearingX / 64.0 - slot->metrics.horiAdvance / 64.0 / 2.0;
