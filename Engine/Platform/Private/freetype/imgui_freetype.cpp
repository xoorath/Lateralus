...
//  2021/03/02: set 'atlas->TexPixelsUseColors = true' to help some backends with deciding of a
//  prefered texture format. 2021/01/28: added support for color-layered glyphs via
//  ImGuiFreeTypeBuilderFlags_LoadColor (require Freetype 2.10+). 2021/01/26: simplified integration
//  by using '#define IMGUI_ENABLE_FREETYPE'.
//              renamed ImGuiFreeType::XXX flags to ImGuiFreeTypeBuilderFlags_XXX for consistency
//              with other API. removed ImGuiFreeType::BuildFontAtlas().
//  2020/06/04: fix for rare case where FT_Get_Char_Index() succeed but FT_Load_Glyph() fails.
//  2019/02/09: added RasterizerFlags::Monochrome flag to disable font anti-aliasing (combine with
//  ::MonoHinting for best results!) 2019/01/15: added support for imgui allocators + added FreeType
//  only override function SetAllocatorFunctions(). 2019/01/10: re-factored to match big update in
//  STB builder. fixed texture height waste. fixed redundant glyphs when merging. support for glyph
...
//  2017/10/22: minor inconsequential change to match change in master (removed an unnecessary
//  statement). 2017/09/26: fixes for imgui internal changes. 2017/08/26: cleanup, optimizations,
//  support for ImFontConfig::RasterizerFlags, ImFontConfig::RasterizerMultiply. 2017/08/16:
//  imported from https://github.com/Vuhdo/imgui_freetype into
//  http://www.github.com/ocornut/imgui_club, updated for latest changes in ImFontAtlas, minor
//  tweaks.
...
#undef DE_MULTIPLY
    ...
    default: IM_ASSERT(0 && "FreeTypeFont::BlitGlyph(): Unknown bitmap pixel mode!");
    }
}
} // namespace
                                    ...
                                    unsigned int extra_flags)
{
    ...
    stbrp_init_target(&pack_context, atlas->TexWidth, TEX_HEIGHT_MAX, pack_nodes.Data,
                      pack_nodes.Size);
