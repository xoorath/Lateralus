...
bool binary_to_compressed_c(string_view inputParam, string_view outputParam, string_view symbolParam, string_view const namespaceParam)
{
    if (inputParam.empty())
    {
        cerr << "[error] input param (-i) is required." << endl;
        return false;
    }
    string outputFilename(outputParam);
    if (outputParam.empty())
    {
        outputFilename = string(inputParam);
        outputFilename = outputFilename.substr(0, outputFilename.find_last_of("."));
        outputFilename += ".cpp"s;
        outputParam = outputFilename;
    }

    bool outputIsCpp = false, outputIsIxx = false;
    {
        size_t lastDot = outputParam.find_last_of(".");
        ...
        else if (outputExtension == "ixx"sv)
        {
            outputIsIxx = true;
        }
        if (symbolParam.empty())
        {
            size_t firstNonSlash = outputParam.find_last_of("/\\");
            if (firstNonSlash == string_view::npos)
            {
                symbolParam = outputParam.substr(0, lastDot);
            }
            else
            {
    ...
    if (!outputIsCpp && !outputIsIxx)
    {
        cerr << "[error] output param must end in .cpp or .ixx" << endl;
        return false;
    }
    ...
    if(!namespaceParam.empty())
    {
        ...
        string_view proccessingNamespace = namespaceParam;
        size_t scopeResOpIndex = proccessingNamespace.find_first_of("::");

        // No scope resolution operators: add the whole string and we're done.
        if (scopeResOpIndex == string_view::npos)
        {
            namespaces.push_back(proccessingNamespace);
            proccessingNamespace = ""sv;
        }
        ...
        while (!proccessingNamespace.empty())
        {
            // Find the next scope res operator.
            scopeResOpIndex = proccessingNamespace.find_first_of("::");
            // If it's at the beginning, we have an invalid namespace like foo::::bar.
            if (scopeResOpIndex == 0)
            {
    ...
    string namespaceSpecifier;
    string exportSpecifier;

    if (!namespaces.empty())
    {
        for (size_t i = 0; i < namespaces.size() - 1; ++i)
        {
            namespaceSpecifier += string(namespaces[i]) + "::"s;
            if (outputIsIxx)
            {
...

    {
        ...
        try
        {
            outputFile.open(string(outputParam));

            string const sizeVarName = string(symbolParam) + "_size"s;

            if (outputIsIxx)
            {
                outputFile << "export module " << exportSpecifier << ";\n";
            }
            outputFile << "//\n//\n";
            ...
            {
                outputFile << "namespace " << namespaceSpecifier << "\n{\n";
                namespaceTabbing = "    ";
            }
            ...
            for (int i = 0; i < compressed.size(); i += 4)
            {
                ...
                outputFile << "0x" << setfill('0') << setw(8) << right << hex << d;
        ...
        {
            cerr << "[Error] Error writing to output file \"" << outputParam << "\"" << endl;
